...
var getAxisFromOptions = NBU.getAxisFromOptions;
 ...
 */
function init() {
    var options = this.options.typeOptions, chart = this.chart, inverted = chart.inverted, xAxis = getAxisFromOptions(chart, 'xAxis', options.xAxis), yAxis = getAxisFromOptions(chart, 'yAxis', options.yAxis), bg = options.background, width = inverted ? bg.height : bg.width, height = inverted ? bg.width : bg.height, selectType = options.selectType;
    if (!xAxis || !yAxis) {
        return;
    }
...
function recalculate(resize) {
    var options = this.options.typeOptions, xAxis = getAxisFromOptions(this.chart, 'xAxis', options.xAxis), yAxis = getAxisFromOptions(this.chart, 'yAxis', options.yAxis), offsetX = this.offsetX, offsetY = this.offsetY;
    if (!xAxis || !yAxis) {
        return;
    }
...
var Measure = /** @class */ (function (_super) {
    ...
    Measure.prototype.addCrosshairs = function () {
        var chart = this.chart, options = this.options.typeOptions, point = this.options.typeOptions.point, xAxis = getAxisFromOptions(chart, 'xAxis', options.xAxis), yAxis = getAxisFromOptions(chart, 'yAxis', options.yAxis), inverted = chart.inverted, defaultOptions = {
            point: point,
            type: 'path'
        };
        if (!xAxis || !yAxis) {
