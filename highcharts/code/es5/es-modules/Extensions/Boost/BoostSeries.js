...
function hasExtremes(series, checkX) {
    var _a;
    var options = series.options, threshold = ((_a = options.boostThreshold) !== null && _a !== void 0 ? _a : Number.MAX_VALUE);
    if (threshold === 0) {
        return false;
    }
...
var getSeriesBoosting = function (series, data) {
    var _a, _b;
    var options = series.options, forceCrop = series.forceCrop, chart = series.chart, threshold = ((_a = options.boostThreshold) !== null && _a !== void 0 ? _a : Number.MAX_VALUE);
    // Return early if either will be grouped or boost is disabled.
    if (forceCrop || threshold === 0) {
        return false;
    }
    return isChartSeriesBoosting(chart) || ((_b = data === null || data === void 0 ? void 0 : data.length) !== null && _b !== void 0 ? _b : 0) >= threshold;
};
/**
...
function scatterProcessData(force) {
    ...
    yCropMin = (_e = yAxis.userMin) !== null && _e !== void 0 ? _e : (isNumber(yAxis.options.min) ?
        yAxis.options.min : -Number.MAX_VALUE), yCropMax = (_f = yAxis.userMax) !== null && _f !== void 0 ? _f : (isNumber(yAxis.options.max) ?
    ...
    var processedXData = [], processedYData = [], processedDataIndices = [], xRangeNeeded = !(isNumber(xExtremes.max) || isNumber(xExtremes.min)), yRangeNeeded = !(isNumber(yExtremes.max) || isNumber(yExtremes.min));
...
function seriesRenderCanvas() {
    ...
    var points = this.points = [], addKDPoint = function (clientX, plotY, i, percentage) {
        ...
        x = xDataFull ? xDataFull[cropStart + i] : false, pushPoint = function (plotX) {
            if (chart.inverted) {
